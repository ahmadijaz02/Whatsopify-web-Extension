...
function handleCreateOrder(request, sender, sendResponse) {
  ...
  .then(({ status, ok, data }) => {
    ...
    if (ok) { // If HTTP status is 2xx, consider it a success
      ...
      const errorMessage = data && (data.detail || data.message) ? (data.detail || data.message) : `API error (Status: ${status})`;
      console.log('[BG] Order creation API call failed (HTTP NOT OK):', errorMessage);
