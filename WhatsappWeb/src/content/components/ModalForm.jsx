...
const ModalForm = ({ onClose }) => {
            ...
            try {
                ...
                if (contactInfoHeader) {
                    ...
                    if (contactInfoPanel) {
                        ...
                        const phoneElement = Array.from(contactInfoPanel.querySelectorAll('span[dir="auto"]'))
                ...
                if (window.getProducts) {
                    ...
                    window.getProducts(products => {
                        if (products && products.length > 0) {
                            ...
                            setSubmissionError('No products found. Please refresh and try again.');
                        }
                        // Set loading to false once the product data (or lack thereof) is received
                        setIsLoading(false);
                    });
                } else {
                    console.warn('⚠️ window.getProducts not available. Ensure index.jsx is loaded correctly.');
                    setSubmissionError('Initialization failed: Product data source not found.');
                ...
                if (window.whatsapofyUserInfo && window.whatsapofyUserInfo.userInfo) {
                    ...
                    console.log('👤 User info loaded from cache:', window.whatsapofyUserInfo.userInfo);
                    ...
                    console.warn('⚠️ window.whatsapofyUserInfo.userInfo not available. Ensure user info is fetched.');
                }
    ...
    useEffect(() => {
        if (selectedProduct && selectedProduct.variants && selectedProduct.variants.length > 0) {
            ...
            setFormData(prev => ({
                ...
                variantId: selectedProduct.variants[0].variantId || selectedProduct.variants[0].id || ''
            }));
        } else {
    ...
    const handleVariantSelect = (e) => {
        ...
        if (selectedProduct && selectedProduct.variants) {
            ...
            const variant = selectedProduct.variants.find(v => (v.id || v.variantId) === variantId);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!currentStoreId) {
                throw new Error("Store ID not available. Please ensure you are logged in and a store is selected/active.");
            }
            ...
            const itemImage = selectedVariant?.image || "https://placehold.co/50x50/cccccc/000000?text=Product";
            ...
            const itemVariantName = selectedVariant?.name || selectedProduct?.name || formData.productName;
            ...
            const orderPayload = {
                ...
                resellerOrder: { // Changed from null to object with required ID and default values
                    accountId: userInfo?.shopilamSurvey?.accountId || "688a4ecae75a76af0b439d13", // Use actual accountId from userInfo or a dummy
                    profit: 0,
                    paidAlready: 0,
                    paymentProof: "",
                    totalCOD: 0,
            ...
            const isActuallySuccessful = success || (data && data.message === 'Order created successfully');
            ...
            if (isActuallySuccessful) {
                ...
                if (window.sendMessageToCurrentChat) {
                    await window.sendMessageToCurrentChat(`✅ Order "${orderIdentifier}" confirmed!`, selectedProduct);
                } else {
                    console.warn('⚠️ window.sendMessageToCurrentChat not available. Message not sent to chat.');
                }
                onClose(); // Close the modal on successful order creation
            } else {
                // Handle order creation failure
                const errorMessage = error || (data && data.detail) || 'Order creation failed with an unknown error.';
    ...
    // If modal root is not found, log an error and return null to prevent rendering
    if (!modalRoot) {
    ...
    const currentProductVariants = selectedProduct && selectedProduct.variants
        ? selectedProduct.variants.filter(v => v.id || v.variantId)
        : [];

    // Render the modal using createPortal to attach it to the modalRoot element
    return createPortal(
        <div style={modalOverlayStyle}>
            <form onSubmit={handleSubmit} style={modalStyle}>
                <div style={scrollableContentStyle}>
                                ...
                                <div style={fieldGroupStyle}>
                                    ...
                                    {selectedProduct && (
                                        <div style={fieldStyle}>
                                            ...
                                            <select
                                                ...
                                                disabled={!selectedProduct || currentProductVariants.length === 0}
                                            >
                                                <option value="">Select a variant</option>
                                                {/* Map through current product's variants */}
                                                {currentProductVariants.map((variant, index) => (
                                                    <option key={variant.id || variant.variantId || index} value={variant.id || variant.variantId}>
                                                        {variant.name || variant.variantId} (SKU: {variant.sku || 'N/A'}) - PKR {variant.price || 'N/A'}
                                                    </option>
                                                ))}
                                            </select>
                                            {errors.variantId && <span style={errorStyle}>{errors.variantId}</span>}
                                        </div>
                                    )}
                                    ...
                                    {selectedVariant && (
                                        <div style={fieldStyle}>
                                            ...
                                            <p style={{ ...inputStyle, backgroundColor: '#f9f9f9', border: '1px dashed #ddd', padding: '8px 12px' }}>
                                                Price: PKR {selectedVariant.price || 'N/A'}<br />
                                                SKU: {selectedVariant.sku || 'N/A'}<br />
                                ...
                                <div style={fieldGroupStyle}>
                                    ...
                                    <div style={fieldStyle}>
                                        ...
                                        <input
                                            ...
                                            style={inputStyle}
                                        />
                                        {errors.province && <span style={errorStyle}>{errors.province}</span>}
                                    </div>
                                    <div style={fieldStyle}>
                    ...
                    {submissionError && (
                        <div style={{ ...errorStyle, padding: '10px', backgroundColor: '#ffebee', borderRadius: '4px', marginTop: '15px' }}>
                            ❌ {submissionError}
                        </div>
                    )}
                </div>
                ...
                <div style={actionsStyle}>
                    ...
                    <button
                        ...
                        disabled={isLoading || isSending}
                    >
                        {isSending ? 'Sending...' : isLoading ? 'Processing...' : 'Place Order'}
                    </button>
                </div>
            </form>
        </div>,
