...
const StoreItem = ({ store }) => {
  ...
  console.log('[STORE] Store status check for:', store.name || store.store_name, '- defaulting to ACTIVE');
  ...
  return (
    <div style={{
      ...
      {/* Store Details */}
      <div style={{ fontSize: '0.85em', color: theme.subText, lineHeight: '1.4' }}>
        {store.description && (
...
const CatalogItem = ({ item }) => {
  ...
  const handleProductClick = () => {
    ...
    const productMessage = `🛒 *${item.name}*\n\n` +
      `💰 *Price:* ${item.price}\n` +
...
const InjectedSidebarContent = ({ catalog = [], stores = [], notes = '', onNotesChange, userInfo = null }) => {
  ...
  useEffect(() => {
    const extractContactInfo = () => {
      try {
        ...
        // Extract Name
        const nameElement = contactInfoPanel.querySelector('span[dir="auto"].x1rg5ohu');
        ...
        // Extract About
        const aboutHeader = Array.from(contactInfoPanel.querySelectorAll('div')).find(el => 
          el.textContent.trim() === 'About'
        );
        const about = aboutHeader?.closest('div')?.nextElementSibling
          ?.querySelector('span[dir="auto"]')?.textContent.trim() || "No status";
    ...
    <div style={{ padding: '20px', fontFamily: 'inherit', background: theme.bg, minHeight: '100vh' }}>
      ...
      {userInfo && (
        <section style={{ marginBottom: '28px' }}>
          <h2 style={{ marginBottom: '12px', fontSize: '1.1rem', color: theme.text }}>User Account</h2>
          <div style={{ background: theme.card, borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.07)', padding: '16px', fontSize: '0.98rem', color: theme.text, border: `1px solid ${theme.border}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              ...
              <strong>Name:</strong>
              <span style={{ color: userInfo?.name ? theme.text : theme.subText }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              <span style={{ fontSize: '1.1em' }}>📧</span>
              <strong>Email:</strong>
              <span style={{ color: userInfo?.email ? theme.text : theme.subText }}>
      ...
      <section style={{ marginBottom: '28px' }}>
        ...
        <div style={{ background: theme.card, borderRadius: '10px', boxShadow: '0 2px 8px rgba(0,0,0,0.07)', padding: '16px', border: `1px solid ${theme.border}` }}>
          {catalog && catalog.length > 0 ? (
            ...
            {/* New Order Button */}
      <section style={{ marginBottom: '28px' }}>
        <h2 style={{ marginBottom: '12px', fontSize: '1.1rem', color: theme.text }}>New Order</h2>
        <button
          onClick={() => setShowOrderForm(true)}
          disabled={isLoading}
          style={{
            background: isLoading ? theme.border : theme.accent,
            color: '#fff',
            border: 'none',
            borderRadius: '6px',
            padding: '10px 16px',
            ...
            ':hover': {
              background: isLoading ? theme.border : '#009688'
            }
          }}
        >
          {isLoading ? 'Loading...' : 'Create New Order'}
        </button>
      </section>
