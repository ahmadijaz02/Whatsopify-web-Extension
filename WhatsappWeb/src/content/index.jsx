...
window.whatsapofyProducts = window.whatsapofyProducts || { catalog: [], storeId: null };
...
let sidebarProps = { contact: {}, catalog: [], stores: [], notes: '', onNotesChange: null, userInfo: null };
...
async function getActiveChatDetails() {
    ...
    if (!chatHeader || !messageArea) {
        console.log('❌ No active chat conversation found (no header or message area)');
        console.log('Header found:', !!chatHeader, 'Message area found:', !!messageArea);
        return null;
    }
    ...
    for (const selector of nameSelectors) {
        ...
        if (nameElement && nameElement.textContent?.trim()) {
            const extractedText = nameElement.textContent?.trim() || nameElement.title?.trim() || '';
    ...
    if (!name || name.toLowerCase().includes('business') || name.toLowerCase().includes('account')) {
        ...
        for (const selector of specificNameSelectors) {
            ...
            if (element && element.textContent && element.textContent.trim()) {
                ...
                if (extractedText &&
                    ...
                    extractedText.length > 1) {
                    name = extractedText;
                    console.log('📝 Found specific name with selector:', selector, '- Name:', name);
                    break;
                }
            }
        }
    ...
    if (!name) {
        console.log('⚠️ No name found with selectors, trying to find title attribute...');
    ...
    const needsMoreInfo = !name || name.toLowerCase().includes('business') || !phone;

    if (needsMoreInfo) {
        console.log('🔍 Need more contact info, attempting to open contact info panel...');
        ...
        if (clickableElement) {
            ...
            while (attempts < maxAttempts) {
                ...
                for (const sel of contactPanelSelectors) {
                    ...
                    if (panel && (panel.textContent?.toLowerCase().includes('contact info') || panel.querySelector('h1, [data-testid="contact-name"]'))) {
                ...
                if (contactInfoPanel) {
                    ...
                    if (!name || name.toLowerCase().includes('business')) {
                        ...
                        for (const selector of nameSelectors) {
                            ...
                            if (nameEl && nameEl.textContent?.trim()) {
                                ...
                                if (!extractedName.toLowerCase().includes('contact') &&
                    ...
                    try {
                        ...
                        const phoneContainers = Array.from(contactInfoPanel.querySelectorAll('[data-testid*="contact-info-phone"], [data-testid*="cell-frame-title"]'));
                        ...
                        if (!phone) {
                            ...
                            const phoneLabel = Array.from(contactInfoPanel.querySelectorAll('div, span')).find(el => 
                    ...
                    const phoneLabels = Array.from(contactInfoPanel.querySelectorAll('div, span')).filter(el => 
                    ...
                    for (const label of phoneLabels) {
                        ...
                        while (nextEl) {
                            ...
                            if (text) {
                                ...
                                const phoneMatch = text.match(/(?:\+|PK\s*)?(\d{2,}[\s\-]?\d{3}[\s\-]?\d{6,})/);
                                if (phoneMatch) {
                    ...
                    const closeButton = contactInfoPanel.querySelector('[data-testid="x"], button[aria-label*="Close"], button[aria-label*="close"], .close, [class*="close"]');
                    if (closeButton) {
                        closeButton.click();
                    } else {
    ...
    if (!phone) {
        console.log('🔍 No phone found, must open contact info panel to get contact phone number...');
        ...
        for (const selector of clickableSelectors) {
            clickableElement = chatHeader.querySelector(selector);
            if (clickableElement) {
                console.log('✅ Found clickable header element for phone extraction:', selector);
                break;
            }
        }
        ...
        if (clickableElement) {
            ...
            while (attempts < maxAttempts) {
                ...
                for (const selector of contactInfoSelectors) {
                    ...
                    if (panel && (panel.textContent?.includes('Contact info') ||
                ...
                if (contactInfoPanel) {
                    ...
                    for (const selector of phoneSelectors) {
                        ...
                        if (phoneEl && phoneEl.textContent) {
                            ...
                            if (phoneMatch && phoneMatch[1].replace(/\D/g, '').length >= 7) {
                                ...
                                console.log('📞 Found contact phone with selector:', selector, '- Phone:', phone);
                                break;
                            }
                        }
                    }
                    ...
                    if (!phone) {
                        const allTextElements = Array.from(contactInfoPanel.querySelectorAll('div, span, p, a'));
                    ...
                    if (closeButton) {
                        ...
                        const backdrop = document.querySelector('div[class*="backdrop"], div[class*="overlay"]');
                        if (backdrop) {
                            backdrop.click();
                        } else {
            ...
            if (attempts >= maxAttempts) {
                console.warn('⚠️ Contact info panel did not appear within timeout for phone extraction');
            }
        } else {
    ...
    try {
        ...
        try {
            ...
            if (whatsopifyTokenRaw) {
                ...
                if (whatsopifyTokenObj && whatsopifyTokenObj.data && whatsopifyTokenObj.data.token) {
                    token = whatsopifyTokenObj.data.token;
                    if (whatsopifyTokenObj.data.user) {
                        console.log('[USER] Found user info in token data:', whatsopifyTokenObj.data.user);
        ...
        const response = await fetch(corsProxy + encodeURIComponent(apiUrl), fetchOptions);
...
function getUserInfo(callback) {
    console.log('[USER] getUserInfo called, userInfoCache:', userInfoCache, 'userInfoLoading:', userInfoLoading);
...
async function fetchStoresForUser() {
    ...
    try {
        ...
        if (storesCache.length > 0) {
            window.whatsapofyProducts.storeId = storesCache[0]._id || storesCache[0].id || storesCache[0].storeId || storesCache[0].store_id;
            console.log('[STORES] Global active store ID set:', window.whatsapofyProducts.storeId);
        } else {
...
function getProducts(callback) {
    ...
    if (productsCache !== null) { // Check for null, not just empty array
...
async function fetchProductsFromAPI() {
    ...
    try {
        ...
        if (storesCache === null && !storesLoading) {
            console.log('[PRODUCTS] Stores cache is null and not loading, fetching stores first...');
            await new Promise((resolve) => {
                getStoresForUser(() => resolve());
            });
        } else if (storesLoading) {
             console.log('[PRODUCTS] Stores are currently loading, waiting for them...');
             await new Promise((resolve) => {
        ...
        const userStoreIds = storesCache ? storesCache.map(store =>
            store._id || store.id || store.storeId || store.store_id
        ).filter(Boolean) : [];

        const corsProxy = 'https://corsproxy.io/?'; // CORS proxy to bypass cross-origin restrictions
        const fetchOptions = { headers: { 'Authorization': `Bearer ${token}` } };
        ...
        try {
            ...
            if (response.ok) {
                ...
                try {
                    ...
                    if (Array.isArray(data)) {
                        productsFromApiResponse = data;
                    } else if (data && data.products && Array.isArray(data.products)) {
                        productsFromApiResponse = data.products;
                    } else if (data && data.data && Array.isArray(data.data)) {
                        productsFromApiResponse = data.data;
                    } else if (data && data.result && Array.isArray(data.result)) {
                        productsFromApiResponse = data.result;
                    } else {
                    ...
                    console.error(`[PRODUCTS] ❌ Invalid JSON response:`, text, err);
                    allUserProducts = []; // Ensure empty array on error
                }
            } else {
            ...
            console.error(`[PRODUCTS] ❌ Network error fetching products:`, err);
            allUserProducts = []; // Ensure empty array on error
        }
        ...
        productsCache = allUserProducts.map(p => {
            const processedProduct = {
                ...
                price: p.price || (Array.isArray(p.variants) && p.variants.length > 0 ? p.variants[0].price : 'N/A'),
                image: (Array.isArray(p.images) && p.images.length > 0) ? p.images[0].url : null,
            };
            return processedProduct;
        });

        console.log(`[PRODUCTS] 📊 Final result: ${productsCache.length} products processed`);
...
function getCatalogForContact(contact, callback) {
    console.log('🛍️ getCatalogForContact called, productsCache:', productsCache, 'productsLoading:', productsLoading);
...
console.log('📝 Loaded notes from storage:', Object.keys(notesMap).length, 'contacts');
...
function handleNotesChange(newNotes) {
    if (sidebarProps.contact && sidebarProps.contact.name) {
        ...
        console.log('📝 Notes updated for contact:', contactKey, '- Length:', newNotes.length);
    }
}
...
function getNotesForContact(contactName) {
    ...
    console.log('📖 Retrieved notes for contact:', contactName, '- Length:', notes.length);
    return notes;
}
...
function observeActiveChat() {
    ...
    if (chatListContainer) {
        ...
        chatListContainer.addEventListener('click', async (event) => {
            ...
            const chatItem = event.target.closest('[data-testid="cell-frame-container"], div[role="listitem"], div[tabindex="0"]');
            if (chatItem) {
                ...
                setTimeout(async () => {
                    ...
                    if (contact && (contact.name || contact.phone || contact.about)) {
                        ...
                        getUserInfo((userInfo) => {
                            ...
                            getStoresForUser((stores) => {
                                ...
                                getCatalogForContact(contact, (products) => {
                                    ...
                                    if (sidebarRoot) {
                                        console.log('🎨 Rendering sidebar with props:', sidebarProps);
                                        sidebarRoot.render(<InjectedSidebarContent {...sidebarProps} />);
                                    } else {
                        ...
                        if (!contact.phone) {
                            console.log('⚠️ No phone number found for contact, but still updating sidebar');
                        }
                        ...
                        console.log('❌ No contact data extracted, keeping current sidebar state');
                    }
                }, 200); // Wait 200ms for chat to load (reduced from 500ms)
            }
        });
    } else {
        console.warn('⚠️ Chat list container not found, falling back to mutation observer');
        ...
        if (chatPanel) {
            ...
            const observer = new MutationObserver(async () => {
                ...
                const hasActiveChat = chatPanel.querySelector('header') && chatPanel.querySelector('[data-testid="conversation-panel-messages"], [role="log"], div[class*="message"]');
                if (hasActiveChat) {
...
window.toggleWhatsappSidebar = async (open) => {
    ...
    if (open !== false) {
        if (!requireAuth(() => {
            ...
            const loginModalContainer = document.getElementById('login-modal-root') || document.createElement('div');
            ...
            loginRoot.render(<LoginModal isOpen={true} onClose={() => loginRoot.unmount()} />);
        })) {
...
function injectSidebarButtons() {
    ...
    waitForElement(leftSidebarHeaderSelector, (flexGrowParentDiv) => {
        ...
        root.render(<InjectedSidebarButtons onToggleSidebar={window.toggleWhatsappSidebar} />);
...
async function addImageToChat(imageFile) {
    try {
        ...
        const photoButton = document.querySelector('[data-testid="mi-attach-photo"]') ||
...
window.sendMessageToCurrentChat = function(message, productItem = null) {
    ...
    try {
        ...
        const chatFooter = document.querySelector('footer[data-testid="conversation-compose-box-input"]') ||
                          document.querySelector('footer') ||
                          document.querySelector('[data-testid="conversation-compose-box-input"]');

        if (chatFooter) {
            // Look for contenteditable div within the footer
            messageInput = chatFooter.querySelector('div[contenteditable="true"]') ||
                          chatFooter.querySelector('[role="textbox"]') ||
                          chatFooter.querySelector('div[data-testid="conversation-compose-box-input"]');
        }
        ...
        if (!messageInput) {
            ...
            for (const selector of selectors) {
                ...
                if (messageInput && messageInput.offsetParent !== null) { // Check if element is visible
        ...
        if (!messageInput) {
            ...
            console.log('[CHAT] Available contenteditable elements:', document.querySelectorAll('[contenteditable="true"]'));
            return false;
        }
        ...
        setTimeout(() => {
            try {
                ...
                const events = [
                    ...
                    new KeyboardEvent('keyup', { bubbles: true, key: 'a', ctrlKey: true }),
                ...
                // Final verification
                setTimeout(() => {
                    const finalContent = messageInput.textContent || messageInput.innerText || messageInput.value;
                    console.log('[CHAT] Final content check:', finalContent);

                    if (finalContent && finalContent.trim().length > 0) {
                        console.log('[CHAT] ✅ SUCCESS! Product details are now in the message input box');
                        console.log('[CHAT] You can now press the send button to share the product');
                        ...
                        console.warn('[CHAT] Content was not set properly, trying alternative method...');

                        // Alternative method: Simulate typing
                        messageInput.focus();
                        const textEvent = new CompositionEvent('compositionstart', { bubbles: true });
                        messageInput.dispatchEvent(textEvent);

                        messageInput.textContent = message;

                        const endEvent = new CompositionEvent('compositionend', { bubbles: true, data: message });
                        messageInput.dispatchEvent(endEvent);

                        const inputEvent = new InputEvent('input', { bubbles: true, inputType: 'insertCompositionText', data: message });
        ...
        if (productItem && productItem.image) {
            console.log('[CHAT] Product has image, attempting to add image to chat...');
            setTimeout(async () => {
                try {
                    const filename = `${productItem.name.replace(/[^a-zA-Z0-9]/g, '_')}.jpg`;
                    const imageFile = await downloadImageAsFile(productItem.image, filename);

                    if (imageFile) {
                        const imageAdded = await addImageToChat(imageFile);
                        if (imageAdded) {
                            console.log('[CHAT] ✅ Product image added to chat successfully!');
                        } else {
...
window.clearProductsCache = function() {
    ...
    console.log('[PRODUCTS] ✅ Products cache cleared. Next sidebar update will fetch fresh data.');
};
...
window.clearStoresCache = function() {
    ...
    console.log('[STORES] ✅ Stores cache cleared. Next sidebar update will fetch fresh data.');
};
...
window.clearUserInfoCache = function() {
    ...
    console.log('[USER] ✅ User info cache cleared. Next sidebar update will fetch fresh data.');
};
...
window.testNewProductsAPI = function() {
    ...
    .then(text => {
        ...
        try {
            ...
            console.log('[PRODUCTS] Number of products:', Array.isArray(data) ? data.length : (data.products ? data.products.length : 'Unknown'));
        } catch (e) {
...
window.debugProductsState = function() {
    ...
    console.log('window.whatsapofyProducts.catalog:', window.whatsapofyProducts.catalog);
    console.log('window.whatsapofyProducts.storeId:', window.whatsapofyProducts.storeId);
    console.log('window.whatsapofyUserInfo.userInfo:', window.whatsapofyUserInfo.userInfo);
};
